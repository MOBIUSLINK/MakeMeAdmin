...
namespace SinclairCC.MakeMeAdmin
{
    ...
    public static class LocalAdministratorGroup
    {
        ...
        /// <summary>
        /// Adds a user to the local Administrators group by SID.
        /// </summary>
        /// <param name="userSid">
        /// The security identifier (SID) of the user to be added.
        /// </param>
        ...
        /// </param>
        public static void AddUserBySid(SecurityIdentifier userSid, DateTime? expirationTime, string remoteAddress)
        {
            if ((LocalAdminGroup != null) && (userSid != null))
            {
                ...
                string userName = Shared.GetAccountNameFromSID(userSid) ?? userSid.Value;
                User user = new User(userSid, userName, expirationTime, remoteAddress);
                
                encryptedSettings.AddUser(user, expirationTime, remoteAddress);

                AddUserToAdministrators(userSid);
            }
        }
        ...
        /// <summary>
        /// Runs the expiration check: removes any users whose administrator rights have expired.
        /// Used by the client service timer and by the user client on exit so that privilege
        /// revocation does not depend on the user app staying open.
        /// </summary>
        public static void RunExpirationCheck()
        {
            EncryptedSettings encryptedSettings = new EncryptedSettings(EncryptedSettings.SettingsFilePath);
            ...
            if (expiredUsers != null)
            {
                foreach (User prin in expiredUsers)
                {
                    ...
                    if ((Settings.EndRemoteSessionsUponExpiration) && (!string.IsNullOrEmpty(prin.RemoteAddress)))
                    {
                        ...
                        {
                            LocalAdministratorGroup.EndNetworkSession(string.Format(@"\\{0}", prin.RemoteAddress), userName);
                        }
                    }
                }
            }
