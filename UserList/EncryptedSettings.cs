namespace SinclairCC.MakeMeAdmin
{
    ...
    public class EncryptedSettings
    {
        ...
        /// <summary>
        /// Adds a user to the list using a User object directly.
        /// </summary>
        /// <param name="user">
        ...
        /// The remote host from which the request for administrator rights came.
        /// </param>
        public void AddUser(User user, DateTime? expirationTime, string remoteAddress)
        {
            if (this.AddedUsers.Contains(user.Sid))
            { // The user is already in the list.
                if (this.AddedUsers[user.Sid].ExpirationTime.HasValue)
                {
                    if (expirationTime.HasValue)
                    {
                        // Choose the earlier of the two expiration times.
                        this.AddedUsers[user.Sid].ExpirationTime = DateTime.FromFileTime(Math.Min(expirationTime.Value.ToFileTime(), this.AddedUsers[user.Sid].ExpirationTime.Value.ToFileTime()));
                    }
                }
                else if (expirationTime.HasValue)
                {
                    this.AddedUsers[user.Sid].ExpirationTime = expirationTime;
                }
            }
            else
            { // The user is not already in the list.
