...
namespace SinclairCC.MakeMeAdmin
{
    ...
    public class Shared
    {
        ...
        /// <summary>
        /// Gets the TCP service address for a specific server.
        /// </summary>
        /// <param name="serverAddress">The server address (IP or hostname).</param>
        /// <returns>The TCP service address.</returns>
        public static string GetTcpServiceAddress(string serverAddress)
        {
            if (string.IsNullOrEmpty(serverAddress))
            {
                serverAddress = "localhost";
            }
            return string.Format("net.tcp://{0}/MakeMeAdmin/Service", serverAddress);
        }
        ...
        public static int GetTimeoutForUser(WindowsIdentity userIdentity)
        {
            ...
            // Check user SID override first (highest priority)
            if (overrides != null && overrides.ContainsKey(userIdentity.User.Value))
            {
                ...
                {
                    // Use override value directly (not Math.Max) to respect admin's custom timeout setting
                    // Note: 0 means permanent (no expiration), any positive value means timeout in minutes
                    timeoutMinutes = overrideMinutes;
                    return timeoutMinutes;
                }
            }

            // Check group SID overrides
            if (overrides != null)
            {
                foreach (SecurityIdentifier sid in userIdentity.Groups)
                {
                    if (overrides.ContainsKey(sid.Value))
                    {
                        ...
                        {
                            // If group override is 0 (permanent), use it directly
                            // Otherwise, use the maximum of current timeout and group override
                            if (overrideMinutes == 0)
                            {
                                timeoutMinutes = 0; // Permanent
                            }
                            else if (overrideMinutes > 0)
                            {
